...
Right now that mostly means **Uzbek-language AI** — collecting and cleaning training data, fine-tuning on rented GPUs, and shipping the product around it. About **35,000 people across 15 countries** have used something I've made.
...
`llm` `fine-tuning` `rag` `evals` `next.js` — ~1,000 users since launch
...
`react` `typescript` `ml` — ~20,000 users · 100,000+ interactions
...
`ocr` `react` `accessibility` — ~15,000 users · 89,000+ events
